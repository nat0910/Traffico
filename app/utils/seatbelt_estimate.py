...
class SeatbeltDetection:
    ...
    def __init__(self):
        ...
        self.seatbelt_model = YOLO('yolo_models/seatbealt/seatbealt-detection-weights.pt')
    ...
    def plot_box_and_track(self, track_id, box, cls, track,path,speed_threshold):
        """
        ...
        if int(cls) == 2 or int(cls) == 5 or int(cls) == 7: 
            ...
            if track_id in self.dist_data:
                speed = self.dist_data[track_id]
                if speed is not None and speed > speed_threshold:
                    if track_id not in self.exported_images:
                        x1, y1, x2, y2 = map(int, box)

                        seatbelt_region = self.im0[y1:y2, x1:x2]

                        results = self.seatbelt_model.predict(seatbelt_region)
                        detected = results[0].boxes.data.detach().cpu().numpy()

                        if len(detected) > 0: 
                        #   print("ho rha hai",detected)

                          for detect in detected:
                                sx1, sy1, sx2, sy2 , scnf ,scls = detect

                                if int(scls) == 2 :
                                    print("No seatbelt")
                                    seatbelt_label = f"{track_id} Not Wearing Seatbelt"
                                    current_datetime = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
                                    image_name = f"track_{track_id}_{current_datetime}_noseatbelt.jpg"
                                    ...
                                    print("seatbelt")
                                    seatbelt_label = f"{track_id}  Wearing Seatbelt"
                                self.annotator.box_label(box, seatbelt_label, bbox_color)


                                
                        # image_name = f"track_{track_id}_speed_{int(speed)}kmph.jpg"
                        # image_path = os.path.join(path, image_name)
                        # cropped_region = self.im0[y1+20:y2+20, x1+20:x2+20]
                        # cv2.imwrite(image_path, cropped_region)
    ...
    def estimate_speed(self, im0, tracks,path=None,speed_threshold=50, region_color=(255, 0, 0)):
        """
        ...
        for box, trk_id, cls in zip(self.boxes, self.trk_ids, self.clss):
            ...
            self.plot_box_and_track(trk_id, box, cls, track,path,speed_threshold)
