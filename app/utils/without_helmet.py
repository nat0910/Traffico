...
class HelmetDetection:
    ...
    def __init__(self):
        ...
        self.helmet_model =YOLO('yolo_models/helmet/helmet-detection-weights.pt')
    ...
    def plot_box_and_track(self, track_id, box, cls, track,path,speed_threshold):
        ...
        # speed_label = f"{int(self.dist_data[track_id])}km/ph" if track_id in self.dist_data else self.names[int(cls)]
            ...
            # speed_label = f"{int(self.dist_data.get(track_id, 0))}km/ph" if track_id in self.dist_data else self.names[int(cls)]
        ...
        if track_id in self.dist_data:
            ...
            if speed is not None and speed > speed_threshold:
                if track_id not in self.exported_images:
                    ...
                    person_bound_box = self.im0[y1:y2, x1:x2]

                    results = self.helmet_model.predict(person_bound_box)
                    detected = results[0].boxes.data.detach().cpu().numpy()
                    if len(detected) > 0 :
                        for detect in detected : 
                            hx1, hy1, hx2, hy2 , hcnf ,hcls = detect
                            print(hcls)
                            if int(hcls) == 1 :
                                if track_id not in self.exported_images:
                                    print("Without Helmet")
                                    helmet_label = f"{track_id} Without-Helmet"
                                    current_datetime = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
                                    image_name = f"track_{track_id}_{current_datetime}_withouthelmet.jpg"
                                    image_path = os.path.join(path, image_name)
                                    cropped_region = self.im0[y1:y2, x1:x2]
                                    cv2.imwrite(image_path, cropped_region)
                                    self.exported_images.add(track_id)
                            else:
                                helmet_label = f"{track_id} Helmet"
                            self.annotator.box_label(box, helmet_label, bbox_color)
